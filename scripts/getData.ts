...
async function main() {
    ...
    fs.readFile( "deploymentScripts/address.json", async (err:any, content:any) => {
        ...
        const NFT = await ethers.getContractFactory("BridgeHistopian");
        const nft = await NFT.attach(json[chainId].bridgeHistopian);
        // const nft = await NFT.deploy("NFT", "NFT", "0xb48dea62c889c6B92992001c2077F17a93eBb00D", 0);
        try {
            console.log("nft mint fee on chain", chainId, await nft.bridgeAttestor());
        } catch (e) {
